...
| `stall_minutes` | `FLACLI_STALL_MINUTES` | give up on a queued transfer whose place in the user's queue has not moved for this long and ask the next candidate; `0` or `off` never gives up (default `30`) |
